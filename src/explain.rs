use crate::{
    Analysis, EClass, ENodeOrVar, HashMap, Id, Language, PatternAst, Rewrite, Subst, UnionFind, Var,
};
...
impl<L: Language> FlatExplanation<L> {
    ...
    fn make_bindings<'a>(
        ...
        pattern: &[ENodeOrVar<L>],
        location: usize,
        bindings: &mut HashMap<Var, &'a FlatExplanation<L>>,
    ) {
        match &pattern[location] {
            ENodeOrVar::Var(var) => {
...
impl<L: Language> Explain<L> {
    ...
    fn check_rewrite<'a, N: Analysis<L>>(
        &self,
        mut current: &'a FlatExplanation<L>,
        mut next: &'a FlatExplanation<L>,
        rewrite: &Rewrite<L, N>,
        is_rewrite_forward: bool,
    ) -> bool {
    ...
    pub(crate) fn add_match(&mut self, eclass: Id, pattern: &PatternAst<L>, subst: &Subst) -> Id {
