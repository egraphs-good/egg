...
More detail can be found in our [paper](https://arxiv.org/abs/2004.03082).
...
an [equivalence relation] over expressions.
...
Here's an e-graph created and rendered by an example `egg`'s [e-graph visualization][dot].
In `egg` and the academic literature, 
...
<style scoped> i { display: inline-block } </style>
<figcaption> An e-graph as it undergoes a series of rewrites. Gray indicates what didn't change. </figcaption>
<figcaption> (1) The initial e-graph for the term <i>(a × 2) / 2</i>. </figcaption>
<figcaption> (2) After applying rewrite <i>x × 2 → x ≪ 1</i>. </figcaption>
<figcaption> (3) After applying rewrite <i>(x × y) / z → x × (y / z)</i>. </figcaption>
<figcaption> (4) After applying rewrites <i>x / x → 1</i> and <i>1 × x → x</i>. </figcaption>
</figure>
...
We say a term _t_ is _represented_ in an e-class _e_ if you can pick a
single e-node from each e-class such that _t_ is in _e_.
A term is represented in the e-graph if it's represented in any e-class.
In the image above, the terms `2 * a`, `a * 2`, and `a << 1` are all
represented in the same e-class and thus are equivalent.
The terms `1`, `(a * 2) / 2`, and `(a << 1) / 2` are represented in
the e-graph, but not in the same e-class as the prior three terms, so
these three are not equivalent to those three.

E-graphs are useful when you have a bunch of very similar expressions,
some of which are equivalent, and you'd like a compactly store them.
This compactness allows rewrite systems based on e-graphs to
...
1. **Uniqueness of e-nodes**

   There do not exist two distinct e-nodes with equal operators and equal
