...
/// 3) Canonical: nodes are in lexicographically minimal topological order
...
impl<L: Language> DagExpr<L> {
    /// Creates a new `DagExpr` from the given nodes and roots.
    ///
    /// It will minimize an canonicalize the DAG to enforce invariants.
    ///
    /// # Panics
    ///
    /// Panics if the provided nodes and roots do not form a valid DAG expression.
    ...
    /// Creates a new `DagExpr` from the given nodes and roots.
    ///
    /// Returns an error if the provided nodes and roots do not form a valid DAG expression.
    ...
    /// Construct a canonical single-node DAG from a leaf node.
    ///
    /// # Panics
    ///
    /// Panics if the provided node has children.
    ...
    ///
    /// The returned [`RecExpr`] will have a single root (the remapped id of the given root).
    /// The returned [`RecExpr`] will not contain duplicate or redundant nodes, but is not
    /// guaranteed to be in lexicographical minimal order.
...
mod tests {
    ...
    #[test]
    fn merge_with_empty_dags() {
        // empty + empty -> empty
        let empty: DagExpr<SymbolLang> = DagExpr::default();
        let merged = empty.merge(&DagExpr::default());
        assert!(merged.is_empty());
        assert!(merged.roots().is_empty());

        // empty + non-empty -> non-empty (same as right)
        let a: RecExpr<SymbolLang> = "x".parse().unwrap();
        let dag_a = DagExpr::from(a);
        let merged2 = DagExpr::default().merge(&dag_a);
        assert_eq!(merged2.nodes(), dag_a.nodes());
        assert_eq!(merged2.roots(), dag_a.roots());

        // non-empty + empty -> non-empty (same as left)
        let merged3 = dag_a.merge(&DagExpr::default());
        assert_eq!(merged3.nodes(), dag_a.nodes());
        assert_eq!(merged3.roots(), dag_a.roots());
    }

    #[test]
    fn merge_multiple_roots_order_preserved() {
        // First multi-root DAG: [x, y]
        let x: RecExpr<SymbolLang> = "x".parse().unwrap();
        let y: RecExpr<SymbolLang> = "y".parse().unwrap();
        let left_multi = DagExpr::from(x).merge(&DagExpr::from(y));

        // Second multi-root DAG: [a, b]
        let a: RecExpr<SymbolLang> = "a".parse().unwrap();
        let b: RecExpr<SymbolLang> = "b".parse().unwrap();
        let right_multi = DagExpr::from(a).merge(&DagExpr::from(b));

        // Merge both; expected root order: x, y, a, b
        let merged = left_multi.merge(&right_multi);
        assert_eq!(merged.roots().len(), 4);

        let r0 = merged.extract_root(merged.roots()[0]);
        let r1 = merged.extract_root(merged.roots()[1]);
        let r2 = merged.extract_root(merged.roots()[2]);
        let r3 = merged.extract_root(merged.roots()[3]);
        assert_eq!(r0.to_string(), "x");
        assert_eq!(r1.to_string(), "y");
        assert_eq!(r2.to_string(), "a");
        assert_eq!(r3.to_string(), "b");
    }

    #[test]
    fn minimize_prunes_unreachable_nodes() {
        // nodes: [x, f(x), y]; roots: [1] -> y is unreachable and should be pruned
        let x = Id::from(0usize);
        let nodes = vec![
            SymbolLang::leaf("x"),         // 0
            SymbolLang::new("f", vec![x]), // 1
            SymbolLang::leaf("y"),         // 2 (unreachable)
        ];
        let dag = DagExpr::new(nodes, vec![Id::from(1usize)]);
        // Should prune unreachable 'y'
        assert_eq!(dag.len(), 2);
        let rec = dag.extract_root(dag.roots()[0]);
        assert_eq!(rec.to_string(), "(f x)");
        assert!(dag.is_valid());
    }

    #[test]
    #[should_panic(expected = "root out of bounds")]
    fn extract_root_panics_on_out_of_bounds() {
        let dag = DagExpr::from_leaf(SymbolLang::leaf("x"));
        // Deliberately out-of-bounds root id
        let _ = dag.extract_root(Id::from(999usize));
    }
}
